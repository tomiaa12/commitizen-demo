// gz-commit.config.js
module.exports = {
  // 不能被直接 push 的目标分支
  forbidDirectPush: ["release"],
  
  // 分支名，分支名必须以这些前缀开始，否则禁止 push
  allowedBranchPrefixes: ['feat/','fix/','perf/','style/','docs/','refactor/','test/','build/','ci/','chore/','revert/',],

  // 禁止合并规则：数组项每项是 { from: [srcPatterns], to: [dstPatterns] }
  // 当本地试图把源分支 from 合并到目标分支 to 时会被阻止
  forbidMerges: [
    // { from: ["uat"], to: ["develop","main"] } // 示例

    // test 禁合并到所有分支
    { from: ["test"], to: [/.*/], msg: "不允许从 test 分支合并到当前分支" }, 

    // gray 只能合并到以 gray 结尾的分支，这里示例为“非以 gray 结尾则禁止”
    // { from: ["gray"], to: [/^((?!.*(gray)$).*)$/], msg: "从 gray 分支合并到当前分支时，当前分支必须以 gray 结尾" },
  ],

  // 提示前缀
  messagePrefix: "[git gz] ",
};
